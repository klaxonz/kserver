...
public class YtDlpDownloaderNew implements Downloader {
    ...
    public void download(String url) {
        try {
            ...
            List<DownloadState> stateList = Lists.newLinkedList();
            ...
            for (VideoInfo videoInfo : videoInfoList) {
                ...
                DownloadState state = callback.onBefore(webPageTask, videoInfo, thumbnailPath);
                stateList.add(state);
            }

            for (int i = 0; i  < videoInfoList.size(); i++) {
                VideoInfo videoInfo = videoInfoList.get(i);
                DownloadState state = stateList.get(i);
